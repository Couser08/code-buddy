...
import { Search, Bell, ChevronDown, Code2, User, Zap } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onOpenAuth }) => {
  ...
  return (
    <header className="h-18 px-6 sm:px-8 flex items-center justify-between border-b border-slate-200/80 bg-[#F8F9FA]/95 sticky top-0 z-30">
      {/* Search Bar */}
      <div className="relative w-full max-w-md">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2 bg-[#F1F5F9]/80 hover:bg-[#E2E8F0]/60 focus:bg-white text-xs sm:text-sm text-slate-800 placeholder-slate-400 rounded-2xl border border-transparent focus:border-blue-500/40 focus:ring-3 focus:ring-blue-500/10 focus:outline-none transition-all"
        />
      </div>

      {/* Right Controls */}
      <div className="flex items-center gap-2 sm:gap-3">
        {/* Lite / Battery Saver Mode Toggle (Auto + Manual 60 FPS Control) */}
        <button
          ...
          title={isLiteMode ? 'Lite Mode ON: GPU blurs and minimaps disabled for 60 FPS' : 'Enable Lite Mode (Recommended for low-end devices / smooth 60 FPS)'}
        >
          <Zap className={`w-3.5 h-3.5 ${isLiteMode ? 'text-amber-600 fill-amber-500' : 'text-slate-400'}`} />
          <span className="hidden md:inline">{isLiteMode ? 'Lite Mode ON' : 'Lite Mode'}</span>
        </button>
