...
export const VisualizerPage: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col overflow-hidden p-3.5 sm:p-4.5 space-y-3 bg-slate-50/50">
      ...
      {/* 2. Concept Selector Dropdown */}
      <div className="relative shrink-0" ref={dropdownRef}>
        {/* Trigger button */}
        <button
          onClick={() => setIsDropdownOpen(!isDropdownOpen)}
          className="w-full flex items-center gap-3 px-4 py-2.5 bg-white border border-slate-200/90 rounded-2xl shadow-2xs hover:border-blue-400/60 hover:shadow-blue-100/50 transition-all cursor-pointer group"
        >
          {/* Icon */}
          ...
          <div className="flex-1 text-left min-w-0">
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none">
              C Concept — Select Topic
            </div>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-sm font-extrabold text-slate-900 truncate">{activeTopic.title}</span>
              <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-md bg-blue-100 text-blue-700 shrink-0">
                {activeTopic.badge}
              </span>
            </div>
          </div>
          ...
          {/* Step count pill */}
          <div className="hidden sm:flex items-center gap-1.5 text-[11px] font-mono text-slate-500 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1 shrink-0">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
            {steps.length} steps
          </div>

          {/* Chevron */}
          <div className={`shrink-0 transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`}>
          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden">
            ...
            <div className="px-4 py-2.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                Choose a C Concept to Visualize
              </span>
              <span className="text-[10px] font-mono text-slate-400">{VISUALIZER_TOPICS.length} topics</span>
            </div>

            {/* Topic list — grouped by category */}
            <div className="max-h-[360px] overflow-y-auto py-2">
              {(() => {
                const groups: Record<string, typeof VISUALIZER_TOPICS> = {};
                ...
                return Object.entries(groups).map(([category, topics]) => (
                  <div key={category}>
                    {/* Category label */}
                    <div className="px-4 py-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50/60">
                      {category}
                    </div>
                    {topics.map(topic => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`w-2 h-2 rounded-full shrink-0 ${isSelected ? 'bg-blue-600' : 'bg-slate-200'}`} />

                          {/* Text */}
                          <div className="flex-1 min-w-0">
                            <div className={`text-sm font-bold truncate ${isSelected ? 'text-blue-800' : 'text-slate-800'}`}>
                              {topic.title}
                            </div>
